...
export default function App() {
  ...
  const activeProjectRef = useRef('');   // ref so save effect reads current value without re-registering
  const skipNextSave     = useRef(false); // suppress the save that fires right after loading
  ...
  // Discover projects from the backend API
  useEffect(() => {
    fetchProjects().then(setProjects).catch(() => setProjects([]));
  }, []);

  // Switch project: load its workflow.json from backend
  ...
  // Auto-save whenever nodes/edges change (debounced 1 s)
  useEffect(() => {
    const project = activeProjectRef.current;
    if (!project) return;

    // Suppress the save triggered by loading a project
    ...
    saveTimer.current = setTimeout(async () => {
      ...
      try {
        await saveWorkflow(currentProject, { nodes, edges });
        setSaveStatus('saved');
  ...
  }, [nodes, edges]); // activeProject intentionally omitted — read via ref
