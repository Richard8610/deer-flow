...
DeerFlow 在企业场景中需要支撑多用户、多个人助手 / custom agent / workflow 的并发使用。部署一整套 DeerFlow 后，需要解决角色权限分级、用户数据隔离、沙箱环境管理和运维伸缩等问题。

> **口径更新（2026-05）**：初期默认每个登录用户均可通过个人助手调用 workflow builder 创建、编辑、发布自己的 workflow；管理员/开发者专属创建权、强审批和部门数据权限作为企业交付阶段的可选收窄能力。

核心澄清点：

1. **角色权限**：初期用户可创建自己的 workflow 并发布到「我的能力」；企业交付可按配置收窄为管理员/开发者发布公共能力。
...
| **开发者（developer）** | 企业收窄模式下发布公共 Agent / workflow；管理所负责业务的 skills |
| **普通用户（user）** | 创建、编辑、发布自己的 workflow；使用已授权或已安装的 Agent / workflow；管理自己的对话线程和数据 |
...
| 创建自己的 workflow | ✅ | ✅ | ✅ |
| 发布到「我的能力」 | ✅ | ✅ | ✅ |
| 发布公共 Agent / workflow | ✅ | ✅（限定范围） | 可配置 |
| 管理 Skills | ✅ | ✅（限定范围） | 自己安装/创建的 custom 能力 |
| 使用 Agent / workflow | ✅ | ✅ | ✅（按授权/已安装） |
| 管理自己的线程/数据 | ✅ | ✅ | ✅ |
| 查看他人线程/数据 | ✅ | ❌ | ❌ |
